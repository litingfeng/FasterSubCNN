function VOCevaldet(network, region_proposal, minoverlap)
...
% write to file
fid = fopen(sprintf('aps_%s_%d.txt', method, minoverlap*100), 'w');
for i = 1:num_cls
    fprintf(fid, '%s %f\n', classes{i}, aps(i));
end
fprintf(fid, 'mAP %f\n', mean(aps));
...
% save to matfile
matfile = sprintf('aps_%s_%d.mat', method, minoverlap*100);
save(matfile, 'recalls', 'precisions', 'aps');
