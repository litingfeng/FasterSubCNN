...
def test_net(net, imdb):
    ...
    for i in xrange(num_images):
        ...
        for j in xrange(1, imdb.num_classes):
            inds = np.where(all_boxes[j][0][:, 4] > thresh[j])[0]
            all_boxes[j][0] = all_boxes[j][0][inds, :]
        ...
        #print 'Evaluating detections'
        if not 'objectnet3d' in imdb.name:
	        if imdb._image_set == 'val':
	            prefix = 'validation'
	        elif imdb._image_set == 'test':
	            prefix = 'test'
	        else:
	            prefix = ''

	        filename = os.path.join(imdb._kitti_path, cfg.SUBCLS_NAME, prefix, 'mapping.txt')
	        assert os.path.exists(filename), \
	                'Path does not exist: {}'.format(filename)

	        mapping = np.zeros(imdb._num_subclasses, dtype=np.float)
	        ...
	        with open(filename, 'wt') as f:
	            ...
	            for cls_ind, cls in enumerate(imdb.classes):
	                ...
	                for k in xrange(dets.shape[0]):
	                    if cfg.TEST.SUBCLS:
	                        subcls = int(dets[k, 5])
	                        cls_name = imdb.classes[imdb.subclass_mapping[subcls]]
