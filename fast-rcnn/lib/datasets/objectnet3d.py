...
class objectnet3d(datasets.imdb):
    def __init__(self, image_set, objectnet3d_path=None):
        datasets.imdb.__init__(self, 'objectnet3d_' + image_set)
        self._image_set = image_set
        self._objectnet3d_path = self._get_default_path() if objectnet3d_path is None \
                            else objectnet3d_path
        self._data_path = os.path.join(self._objectnet3d_path, 'Images')
        ...
        assert os.path.exists(self._objectnet3d_path), \
                'objectnet3d path does not exist: {}'.format(self._objectnet3d_path)
    ...
    def _load_image_set_index(self):
        ...
        """
        image_set_file = os.path.join(self._objectnet3d_path, 'Image_sets', self._image_set + '.txt')
    ...
    def _get_default_path(self):
        """
        Return the default path where objectnet3d is expected to be installed.
        """
        return os.path.join(datasets.ROOT_DIR, 'data', 'ObjectNet3D')
    ...
    def gt_roidb(self):
        ...
        gt_roidb = [self._load_objectnet3d_annotation(index)
    ...
    def _load_objectnet3d_annotation(self, index):
        """
        Load image and bounding boxes info from txt file in the objectnet3d format.
        """

        if self._image_set == 'test' or self._image_set == 'test_1' or self._image_set == 'test_2':
            lines = []
        else:
            filename = os.path.join(self._objectnet3d_path, 'Labels', index + '.txt')
            lines = []
    ...
    def _load_rpn_roidb(self, gt_roidb, model):
        ...
        for ix, index in enumerate(self.image_index):
            filename = os.path.join(self._objectnet3d_path, 'region_proposals', model, index + '.txt')
    ...
    def evaluate_detections(self, all_boxes, output_dir):
        ...
        for im_ind, index in enumerate(self.image_index):
            ...
            print 'Writing objectnet3d results to file ' + filename
...
if __name__ == '__main__':
    d = datasets.objectnet3d('trainval')
    res = d.roidb
