...
class ProposalTargetLayer(caffe.Layer):
    ...
    def forward(self, bottom, top):
        ...
        #print "forward ProposalTargetLayer0"
        ...
        sublabels, labels, rois, bbox_targets, bbox_inside_weights = _sample_rois(
        ...
        # add subclass labels
        if cfg.TRAIN.SUBCLS:
            sublabels = sublabels.reshape((sublabels.shape[0], 1, 1, 1))
            top[5].reshape(*sublabels.shape)
            top[5].data[...] = sublabels

        #print "sublabels shape: ", sublabels.shape
        #print "forward ProposalTargetLayer"
...
def _sample_rois(all_rois, gt_boxes, fg_rois_per_image, rois_per_image, num_classes):
    ...
    sublabels = gt_boxes[gt_assignment, 5]
    # Select foreground RoIs as those with >= FG_THRESH overlap
    #fg_inds = np.where(max_overlaps >= cfg.TRAIN.FG_THRESH)[0]
    fg_inds = []
    for i in xrange(1, num_classes):
        fg_inds.extend(np.where((labels == i) & (max_overlaps >= cfg.TRAIN.FG_THRESH[i-1]))[0])
    ...
    bg_inds = []
    for i in xrange(1, num_classes):
        bg_inds.extend( np.where((labels == i) & (max_overlaps < cfg.TRAIN.BG_THRESH_HI[i-1]) &
                        (max_overlaps >= cfg.TRAIN.BG_THRESH_LO[i-1]))[0] )
    # Compute number of background RoIs to take from this image (guarding
    # against there being fewer than desired)
    bg_rois_per_this_image = rois_per_image - fg_rois_per_this_image
    bg_rois_per_this_image = min(bg_rois_per_this_image, len(bg_inds))
    ...
    #print "sublabels shape: ", sublabels.shape
    return sublabels, labels, rois, bbox_targets, bbox_inside_weights
